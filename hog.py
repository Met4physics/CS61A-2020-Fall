...
def swine_align(player_score, opponent_score):
    ...
    if player_score == 0 or opponent_score == 0:
        return False
    temp = min(player_score, opponent_score)
    while temp > 0:
        if player_score % temp == 0 and opponent_score % temp == 0:
            break
        temp -= 1
    if temp > 9:
        return True
...
def pig_pass(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4b
    "*** YOUR CODE HERE ***"
    if player_score - opponent_score < 0 and player_score - opponent_score > -3:
        return True
    return False
    # END PROBLEM 4b
