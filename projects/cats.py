...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def fun(string):
        list = split(lower(remove_punctuation(string)))
        for i in topic:
            for k in list:
                if k == i:
                    return True
...
def accuracy(typed, reference):
    ...
    while i < min(len(reference_words), len(typed_words)):
        if typed_words[i] == reference_words[i]:
            sum += 1
        i += 1
    return 100*sum/len(typed_words)
    # END PROBLEM 3
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    for i in valid_words:
        if diff_function(user_word, i, limit) < diff and diff_function(user_word, i, limit) <= limit:
...
def shifty_shifts(start, goal, limit):
    ...
    def f(start, goal, limit):
        if start == goal:
            return 0
        if limit == 0:
            return 1
        if start == '':
            return 1 + f('', goal[1:], limit-1)
            ...
            return 1 + f(start[1:], goal[1:], limit-1)
        else:
            return f(start[1:], goal[1:], limit)

    if len(start) <= len(goal):
        return f(start, goal, limit)
    else:
        return f(goal, start, limit)
    # END PROBLEM 6
...
def report_progress(typed, prompt, user_id, send):
    ...
    while n < min(len(prompt), len(typed)):
        if typed[n] == prompt[n]:
            ...
            break
        n += 1
    dic = {'id':user_id, 'progress':count / len(prompt)}
    send(dic)
...
def fastest_words(game):
    ...
    for i in word_indices:
        temp = [time(game, n, i) for n in player_indices]
        n = 0
        while n < len(player_indices):
            if temp[n] == min(temp):
                t[n].append(all_words(game)[i])
                break
            n += 1
    return t
    # BEGIN PROBLEM 10
