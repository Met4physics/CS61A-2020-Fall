...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while n < self.min_range:
            ...
            if t is beehive:
                return None
        while len(t.bees) == 0 and n <= self.max_range:
            t = t.entrance
            n += 1
            if t is beehive:
                return None
        if n > self.max_range:
            return None
        return rANTdom_else_none(t.bees) # REPLACE THIS LIN
        # END Problem 3 and 4
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.first == False:
            ...
            while t.exit != None:
                t = t.exit
                if t.ant != None and (not hasattr(t, 'buffered')):
                    t.ant.damage *= 2
                    t.buffered = True
        # END Problem EC
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        # END EC
        if hasattr(self, 'backward') and self.backward:
...
def make_slow(action, bee):
    ...
    "*** YOUR CODE HERE ***"
    def f(self, gamestate):
        if gamestate.time % 2 == 0:
            action(gamestate)

    return lambda x: f(bee, x)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        temp = self.place
        d = 0
        while temp is not beehive:
            if temp.ant is not self and temp.ant:
                t[temp.ant] = d
            for i in temp.bees:
                t[i] = d
            temp = temp.entrance
